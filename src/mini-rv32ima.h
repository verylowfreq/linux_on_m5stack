...
MINIRV32_DECORATE int32_t MiniRV32IMAStep( struct MiniRV32IMAState * state, /*uint8_t * */ RAMDevice& image, uint32_t vProcAddress, uint32_t elapsedUs, int count );
...
MINIRV32_DECORATE int32_t MiniRV32IMAStep( struct MiniRV32IMAState * state, /*uint8_t * */ CachedRAM& image, uint32_t vProcAddress, uint32_t elapsedUs, int count )
{
	...
	for( icount = 0; icount < count; icount++ )
	{
		...
		{
			ir = MINIRV32_LOAD4_PROGRAM( ofs_pc );
